...
document.addEventListener("click", e => {
    ...
    if (btn.id === "go-learn") { game.flow = "learn"; showScreen("screen-modes"); }
    if (btn.id === "go-play") { game.flow = "play"; showScreen("screen-modes"); }

    if (btn.id === "back-to-start") showScreen("screen-start");
    if (btn.id === "back-from-regions") showScreen("screen-modes");
...
async function initGame(region) {
    ...
    L.tileLayer("https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png").addTo(game.map);
    ...
    game.questions = geo.features;

    const layer = L.geoJSON(geo, {
        onEachFeature: (f, l) => {
            l.on("click", () => {
                const name = normalize(f.properties.nombre || f.properties.name);
                alert(name);
            });
        }
    }).addTo(game.map);
